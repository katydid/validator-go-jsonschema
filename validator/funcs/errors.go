...
func (this ErrRangeCheck) Error() string {
	return fmt.Sprintf("validator/funcs: range check error: trying to access index %d in list of length %d", this.Index, this.Len)
}
...
func (this ErrRange) Error() string {
	return fmt.Sprintf("validator/funcs: range error: first %d > last %d", this.First, this.Last)
}
...
func (ErrContainsListNotConst) Error() string {
	return fmt.Sprintf("validator/funcs: list passed to contains is not a constant")
}
